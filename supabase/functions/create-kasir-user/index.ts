...
serve(async (req) => {
  ...
  try {
    ...
    // 4. Pastikan caller adalah admin dan ambil id_toko-nya
    ...
    if (profileErr || callerProfile?.role !== "admin") {
      return new Response(
        JSON.stringify({
          error: "Forbidden: hanya admin toko yang dapat membuat kasir",
        }),
        {
          headers: { ...corsHeaders, "Content-Type": "application/json" },
          status: 403,
    ...
    const callerTokoId = callerProfile.id_toko;

    if (!callerTokoId) {
      return new Response(
        JSON.stringify({ error: "Admin tidak memiliki toko yang valid" }),
        {
          headers: { ...corsHeaders, "Content-Type": "application/json" },
          status: 400,
        },
    ...
    // 7. Buat user_profile dengan role kasir, menggunakan id_toko dari server (bukan dari client)
    const { error: profileInsertErr } = await supabaseAdmin
      .from("user_profiles")
      .insert({
        id: authData.user.id,
        id_toko: callerTokoId, // Selalu menggunakan toko milik caller — tidak bisa dimanipulasi
        role: "kasir",
        nama,
        email,
      });
    ...
    return new Response(
      JSON.stringify({
        success: true,
        user: { id: authData.user.id, email, nama },
      }),
      {
        headers: { ...corsHeaders, "Content-Type": "application/json" },
        status: 200,
    ...
    console.error("Error di create-kasir-user:", msg);
